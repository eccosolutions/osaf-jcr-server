...
public class ReportType implements DeltaVConstants, XmlSerializable {
     ...
     * @throws DavException
     */
    public Report createReport(DavResource resource, ReportInfo info) throws DavException {
        try {
            Report report = (Report) reportClass.newInstance();
            report.init(resource, info);
            return report;
        } catch (IllegalAccessException e) {
            throw new DavException(DavServletResponse.SC_INTERNAL_SERVER_ERROR, "Failed to instantiate report class " + reportClass);
